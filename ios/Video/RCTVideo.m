...
@implementation RCTVideo
{
    ...
    /* Store PHImageRequestID */
    PHImageRequestID _phImageRequestID;
    NSOperationQueue* _setSrcQueue;
...
- (instancetype)initWithEventDispatcher:(RCTEventDispatcher *)eventDispatcher
{
    if ((self = [super init])) {
        ...
        _setSrcQueue = [[NSOperationQueue alloc] init];
        _phImageRequestID = PHInvalidImageRequestID;
...
- (void)stopExistingRequests
{
    if (self->_phImageRequestID != PHInvalidImageRequestID) {
        [[PHImageManager defaultManager] cancelImageRequest:self->_phImageRequestID];
        self->_phImageRequestID = PHInvalidImageRequestID;
    }
}
...
- (void)setSrc:(NSDictionary *)source
{
    ...
    [self removePlayerItemObservers];
    [self stopExistingRequests];
    
    //    [_setSrcQueue addOperationWithBlock:^{
    ...
    [self playerItemForSource:source withCallback:^(AVPlayerItem * playerItem) {
        __strong RCTVideo* strongSelf = weakSelf;
        strongSelf->_playerItem = playerItem;
        [strongSelf addPlayerItemObservers];
        [strongSelf setFilter:strongSelf->_filterName];
        
        [strongSelf->_player pause];
        [strongSelf->_playerViewController.view removeFromSuperview];
        strongSelf->_playerViewController = nil;
        
        if (strongSelf->_playbackRateObserverRegistered) {
            [strongSelf->_player removeObserver:strongSelf forKeyPath:playbackRate context:nil];
            strongSelf->_playbackRateObserverRegistered = NO;
        }
        if (strongSelf->_isExternalPlaybackActiveObserverRegistered) {
            [strongSelf->_player removeObserver:strongSelf forKeyPath:externalPlaybackActive context:nil];
            strongSelf->_isExternalPlaybackActiveObserverRegistered = NO;
        }
        
        strongSelf->_player = NULL;
        ...
        [strongSelf->_player addObserver:strongSelf forKeyPath:playbackRate options:0 context:nil];
        strongSelf->_playbackRateObserverRegistered = YES;
        
        [strongSelf->_player addObserver:strongSelf forKeyPath:externalPlaybackActive options:0 context:nil];
        strongSelf->_isExternalPlaybackActiveObserverRegistered = YES;
        
        [strongSelf addPlayerTimeObserver];
        
        //Perform on next run loop, otherwise onVideoLoadStart is nil
        if (strongSelf.onVideoLoadStart) {
            id uri = [source objectForKey:@"uri"];
            id type = [source objectForKey:@"type"];
            strongSelf.onVideoLoadStart(@{@"src": @{
...
- (void)playerItemForSource:(NSDictionary *)source withCallback:(void(^)(AVPlayerItem *))handler
{
    ...
    if (isNetwork) {
        ...
        if([uri hasPrefix:@"assets-library://"]){
            ...
            self->_phImageRequestID = [[PHImageManager defaultManager] requestAVAssetForVideo:asset
                                                                                      options:options
                                                                                resultHandler:^(AVAsset * _Nullable asset,
                                                                                                AVAudioMix * _Nullable audioMix,
                                                                                                NSDictionary * _Nullable info)
                                       {
                                           [weakSelf playerItemPrepareText:asset assetOptions:assetOptions withCallback:handler];
                                       }];
        }
        return;
    }
    
    AVURLAsset *asset = [AVURLAsset URLAssetWithURL:[[NSURL alloc] initFileURLWithPath:[[NSBundle mainBundle]
                                                                                        pathForResource:uri ofType:type]]
                                            options:nil];
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        [self playerItemPrepareText:asset assetOptions:assetOptions withCallback:handler];
    }];
